...
1		    			--><div class='name-article'>
		    				<h3><nobr><a class='art-link' href='index.php?page=list&sc=<?php echo"{$sc}";?>&art=<?php echo"{$list[$i]->getCode()}";?>' alt='<?php echo "{$list[$i]->getTitulo()} {$list[$i]->getSinopsis()}";?>'><?php echo "{$list[$i]->getTitulo()} {$list[$i]->getSinopsis()}";?></a></nobr></h3> <!-- marca e modelos-->
