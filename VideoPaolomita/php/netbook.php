<?php
	...
	$list = array();
	$query = "SELECT * FROM computer WHERE monitor>0 AND monitor<15 ORDER BY code ASC";
	...
	?>
		            ...
		            <div class='box-article'>
		    			...
		    			--><div class='name-article'>
		    				<h3><nobr><a class='art-link' href='index.php?page=netbook&art=<?php echo"{$list[$i]->getCode()}";?>' alt='<?php echo "{$list[$i]->getMarca()} {$list[$i]->getModelo()}";?>'><?php echo "{$list[$i]->getMarca()} {$list[$i]->getModelo()}";?></a></nobr></h3> <!-- marca e modelo-->
		    				<p> Cod. <?php echo "{$list[$i]->getCode()}"; ?>
		    			<?php	if ($list[$i]->getNum() > 0){
		    					echo " - <font color='#99f614'><nobr>DISPONIBLE</nobr></font></p>";
		    				}
		    				else
		    					echo " - <font color='#ff3636'><nobr>NO DISPONIBLE</nobr></font></p>"; ?>
		    	<!--	echo "-->
		    				</p> <!-- codice -->
		    			</div><!--
